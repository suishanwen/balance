...
class SpotAPI(Client):
    ...
    # take order
    def take_order(self, side, instrument_id, order_type, price, size):
        params = {'side': side, 'instrument_id': instrument_id, 'order_type': order_type,
    ...
    # query depth
    def get_depth(self, instrument_id, size=10, depth=''):
        params = {}
        if size:
            params['size'] = size
        if depth:
