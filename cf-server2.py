...
CONFIG_FILE = "huobi/config.ini"
LOG_FILE = "huobi/log.txt"
RUNNING_LOG_FILE = "huobi/nohup.out"
...
cd codegen
python3 dec.py
echo '----- kill huobiclient -----'
ps -ef | grep HuobiClient.py | grep -v grep | awk '{print $2}' | xargs kill -9
echo '----- start huobiclient -----'
cd /home/balance/huobi
cat /dev/null > nohup.out
nohup python3 HuobiClient.py>/home/balance/huobi/nohup.out 2>&1 &"""
    subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    yield "ok".encode('utf-8')
...
def shutdown(_, start_response):
    start_response('200 OK', [('Content-type', 'text/html')])
    cmd = """ps -ef | grep HuobiClient.py | grep -v grep | awk '{print $2}' | xargs kill -9
"""
    subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    yield "ok".encode('utf-8')
...
@require_auth
def eos_restart(_, start_response):
    start_response('200 OK', [('Content-type', 'text/html')])
    cmd = """echo '----- pull code -----'
cd /home/earn-eos
git pull
ps -ef | grep index.js | grep -v grep | awk '{print $2}' | xargs kill -9
cat /dev/null > nohup.out
nohup node index.js>/home/earn-eos/nohup.out 2>&1 &
"""
    subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    yield "ok".encode('utf-8')
