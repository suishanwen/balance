...
def modify_trans_by_price(_avg_buy, _avg_sell, _next_buy, _next_buy_transaction, _next_sell, _next_sell_transaction,
    ...
    buy_transaction_rate = _next_buy_transaction / client.transaction
    if buy_rate > 1 and buy_rate > buy_transaction_rate:
        return client.transaction * buy_rate, round(client.currentBase / math.pow(client.rateP, buy_rate),
    ...
    if sell_rate > 1 and sell_rate > sell_transaction_rate:
        return _next_buy_transaction, _next_buy, \
               client.transaction * sell_rate, round(client.currentBase * math.pow(client.rateP, sell_rate), 4)
    return _next_buy_transaction, _next_buy, _next_sell_transaction, _next_sell
...
def get_trigger(client, symbol):
    ...
    prev = data[1]
    prev_grand = data[2]
    ma = round(sum1 / len(data1) - sum2 / len(data2), client.ACCURACY + 2)
    # reverse
    ...
    return ma, reverse and (hammer or pierce)
    ...
    try:
        while True:
            if counter > 300:
                ...
                ma, reverse = get_trigger(client, symbol)
                ...
                logger.info(
                    "\nbase:{},ma:{},nextSell:[{},{}] - buy:[{},{}] (+{}) | nextBuy:[{},{}] - sell:[{},{}]({})".format(
                        ...
                        round(
                            ...
                            4)))
            order_info = None
            if (ma > 0 or reverse) and next_buy_p >= avg_sell and sell_amount >= next_buy_amount:
            ...
            elif (ma < 0 or reverse) and next_sell_p <= avg_buy and buy_amount >= next_sell_amount:
                if ma < 0:
                    trigger = "ma"
                else:
                    trigger = "reverse"
