...
def get_next_buy_sell_info(client, symbol):
    ...
    _next_buy_trans = client.transaction * buy_rate
    _next_sell_trans = client.transaction * sell_rate
...
def modify_trans_by_price(_avg_buy, _avg_sell, _next_buy, _next_buy_transaction, _next_sell, _next_sell_transaction,
    ...
    if sell_rate > 1 and sell_rate > sell_transaction_rate:
        return _next_buy_transaction, _next_buy, round(sell_rate * client.transaction, 3), round(
...
def __main__(client, symbol):
    ...
    next_buy, next_buy_trans, next_sell, next_sell_trans = get_next_buy_sell_info(client, symbol)
    while True:
        try:
            if counter > 300:
                next_buy, next_buy_trans, next_sell, next_sell_trans = get_next_buy_sell_info(client, symbol)
